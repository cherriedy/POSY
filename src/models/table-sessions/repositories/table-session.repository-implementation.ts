...
import { TableSession, TableSessionMapper } from '../types';
...
export class TableSessionRepositoryImpl implements TableSessionRepository {
  ...
  async create(entity: TableSession): Promise<TableSession> {
    const prismaTableSession = TableSessionMapper.toPrisma(entity);
    try {
  ...
  /**
   * Deletes a table session by its unique ID.
  ...
  async delete(id: string): Promise<void> {
    try {
      await this.prismaService.tableSession.delete({
        where: { id },
      });
    } catch (e) {
      if (e instanceof PrismaClientKnownRequestError) {
  ...
  async update(
    ...
    // Build update data manually to avoid mapper issues with partial entities
    const dataToUpdate: any = {};

    if (entity.status) dataToUpdate.status = entity.status;
    if (entity.endAt !== undefined) dataToUpdate.end_at = entity.endAt;
    if (entity.expiresAt) dataToUpdate.expires_at = entity.expiresAt;
    if (entity.deviceFingerprint)
      dataToUpdate.device_fingerprint = entity.deviceFingerprint;
