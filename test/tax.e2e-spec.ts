...
const mockLogger = {
  ...
  // Print errors to stdout so failing tests reveal the server error stack
  error: (err: any) => {
...
// Helper to create Prisma error mocks
function prismaError(code: string, message = '') {
  ...
  describe('POST /taxes', () => {
    ...
    it('returns 400 on duplicate name', async () => {
      mockPrisma.taxConfig.create.mockRejectedValue(
        prismaError('P2002', 'Unique constraint violation'),
      );
