...
export class ResetPasswordService {
   ...
   * @throws {UserNotFoundError} If no user is found with the extracted email.
   * @throws {ResetTokenExpiredError} If the reset token has expired.
   * @throws {InvalidResetTokenError} If the reset token is invalid or malformed.
   ...
   * If the token is expired, it throws a `ResetTokenExpiredError`.
   * If the token is invalid or malformed, it throws an `InvalidResetTokenError`.
   *
   * @param {string} token - The JWT reset token to verify.
   * @returns {string} The email address extracted from the valid reset token.
   * @throws {ResetTokenExpiredError} If the token has expired.
   * @throws {InvalidResetTokenError} If the token is invalid or malformed.
   */
